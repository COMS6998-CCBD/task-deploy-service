...
class RDSManager:
    ...
    def insert_task(self, tdr: TaskDeployRequest):
        with RDSConnection() as rc:
            rc.execute(
                "insert into task_request_info (task_id, user_id, task_name, s3_bucket, source_s3_prefix, destination_s3_prefix, command, linux_dependencies) values (%s, %s, %s, %s, %s, %s, %s)",
                (tdr.task_id, tdr.user_id, tdr.task_name, tdr.s3_bucket, tdr.source_s3_prefix, tdr.destination_s3_prefix, tdr.command, " ".join(tdr.linux_dependencies)))
